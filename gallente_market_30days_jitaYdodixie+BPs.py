...
def get_historical_average_price_single(type_id, region_id, price_type='sell'):
    """
    Obtiene el precio de venta ('sell') o de compra ('buy') promedio (AVG Price) 
    del ítem durante los últimos 30 días en una región específica usando la API de History de ESI.
    """
    ...
    try:
        ...
        # ESI History no tiene 'min'/'max'/'average' para buy/sell. 
        # Si queremos precio de COMPRA histórico (proxy para BPC), usaremos el precio más bajo de las ventas 
        # (day_data.get('lowest')) como un indicador conservador para el valor de compra histórico, 
        # o 'average' para el precio de venta. Dado que la ESI History no diferencia Buy/Sell, 
        # usaremos el 'average' para ambos, pero lo marcamos como 'Avg Buy' para BPC.
        
        price_key = 'average' # Usamos el promedio diario para la tendencia, tanto para Venta como Compra.
...
def get_best_historical_price(type_id, price_type='sell'):
    """
    Compara el precio promedio de 30 días entre Jita y Dodixie (Sell para productos, Buy para BPC)
    y devuelve el más alto.
    """
    if price_type == 'sell':
        label = "Venta"
        suffix = " (30D)"
        ...
        label = "Compra"
        suffix = " Avg (30D)" # Lo llamamos Avg Buy para diferenciar del precio de compra actual

    price_jita, _ = get_historical_average_price_single(type_id, JITA_REGION_ID, price_type)
    price_dodi, _ = get_historical_average_price_single(type_id, DODIXIE_REGION_ID, price_type)
    
    best_price = max(price_jita, price_dodi)
    
    if best_price == price_jita and best_price > 0:
        source = f"Jita {label}{suffix}"
    elif best_price == price_dodi and best_price > 0:
        source = f"Dodi {label}{suffix}"
    else:
        source = "N/A"
...
def get_blueprint_materials(bpc_type_id):
    ...
    # Mantenemos esta función para el cálculo de fabricación
...
def calculate_lp_cost_and_profit(offer):
    ...
    for item in required_items:
        ...
        # Costo de adquisición de materiales: Usamos precio de compra ACTUAL de Jita
        buy_price = get_current_market_buy_price(required_id, JITA_REGION_ID)
        total_isk_cost += buy_price * quantity

    # Usamos el mejor precio promedio de VENTA (30D) entre Jita y Dodixie
    historical_avg_price, source = get_best_historical_price(type_id, price_type='sell')
...
def calculate_bpc_resale_profit(offer):
    # Lógica de reventa de Planos (BPC)
    type_id = offer['type_id']
    lp_cost = offer.get('lp_cost', 0)
    ...
    # *** PUNTO CLAVE: Usamos el mejor precio promedio de COMPRA (30D) entre Jita y Dodixie ***
    # Esto es un PROXY para la reventa por contrato.
    bpc_avg_buy_price, source = get_best_historical_price(type_id, price_type='buy')
    
    if bpc_avg_buy_price == 0.0:
        return None, f"Descartada: {item_name}. BPC sin datos históricos de compra."
        
    # Asumimos que el ingreso es el precio de compra promedio. No se aplica impuesto aquí.
    ...
    return {
        ...
        "Avg Sell Price (30D)": bpc_avg_buy_price, # Reutilizamos la columna para mostrar el proxy
    }, None

def calculate_bpc_manufacturing_profit(offer):
    # Lógica de Fabricación (Sin cambios en el cálculo de fabricación)
    ...
    # Usamos el mejor precio promedio de VENTA (30D) entre Jita y Dodixie
    historical_avg_price, source = get_best_historical_price(product_type_id, price_type='sell')
...
def main():
    print("--- 💸 Análisis de Tendencia Multi-Mercado + Reventa BPC V4 💸 ---")
    print("Nota: Usa el precio promedio de 30 días MÁS ALTO entre Jita y Dodixie (Venta para productos, Compra para BPC).")
    ...
    try:
        ...
        print(f"Consultando {len(offers)} ofertas de la tienda LP (triple consulta de precios por BPC)...")
        
        for i, offer in enumerate(offers):
            # Aumentamos la pausa para evitar bloqueos por la mayor cantidad de consultas
            time.sleep(0.15) 
            ...
            if 'lp_cost' in offer and offer['lp_cost'] > 0 and ('isk_cost' in offer or offer.get('required_items')):
                ...
                if is_blueprint:
                    # 1. Reventa de BPC (Lo más simple que se puede hacer con el plano)
                    resale_result, error_resale = calculate_bpc_resale_profit(offer)
                    if resale_result:
                        all_results.append(resale_result)
                    
                    # 2. Fabricación (Usando el plano)
                    manuf_result, error_manuf = calculate_bpc_manufacturing_profit(offer)
                    if manuf_result:
                        all_results.append(manuf_result)
                    
                    # Si ambos fallaron, registramos el error más importante (Fabricación)
                    if not resale_result and not manuf_result:
                         if error_manuf: debug_log.append(error_manuf)
                         elif error_resale: debug_log.append(error_resale)
        ...
        print("------------------------------------------------------------------------------------------------------------------------------------------")
        print(f"{'Item (Método / Mercado)':<60} | {'ISK/LP':<8} | {'LP Cost':<8} | {'ISK Cost':<15} | {'Profit Est.':<15} | {'Avg Sell/Buy Price (30D)':<15}")
        print("------------------------------------------------------------------------------------------------------------------------------------------")
